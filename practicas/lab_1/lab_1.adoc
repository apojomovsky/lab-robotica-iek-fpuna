...
Esto nos llevará a un terminal CMD de Windows cuyo promptfootnote:[https://en.wikipedia.org/wiki/Command-line_interface#Command_prompt] tendrá la particularidad de estar precedido, entre paréntesis, por el nombre de nuestro environment de Anaconda. Esto se hace para enfatizar y recordar que estamos en un entorno confinado, y que cualquier cambio que hagamos en la instalación de Python en este terminal, se hará solo en dicho entorno y no a nivel de sistema.
